...
class PaymentController extends Controller
{
        ...
        if ($pays == null) {
            ...
            if (
                ...
                if ($request->status_message == "settlement") {
                    $detail = Detail_transaksi::where('kode_tiket', $request->order_id)->get();
                    foreach ($detail as $dt => $detail) {
                        $detail->status = 1;
                        $detail->save();
                    }
                    if ($detail->kategori = "event") {
                        $eventkeg = Kegiatan::where('kode_kegiatan', $detail->id_produk)->first();
                        $kode_event = $detail->id_produk;

                        if ($eventkeg->kapasitas_b >= $eventkeg->kapasitas) {
                            return redirect()->route('pesananku');
                        }
                        $eventkeg->kapasitas_b += (int)$detail->jumlah;

                        // dd($eventkeg->kapasitas_b);
    ...
    public function pesananku()
    {
        ...
        //     $tiket = Tiket::where('user_id', Auth::user()->id)->orderby('id', 'desc')->where('status', 0)->get();
        //     // $deskripsi =
    ...
    public function pesananku_detail($kode)
    {
        ...
        if (Auth::user()->status == '1') {
            ...
            // $dec = Detail_camp::where('kode_tiket', $kode)->first();
